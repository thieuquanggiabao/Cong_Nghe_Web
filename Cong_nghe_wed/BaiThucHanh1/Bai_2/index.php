<?php
// Đọc file Quiz.txt
$filename = "Quiz.txt";
$content = file_get_contents($filename);

// Tách từng block câu hỏi
$blocks = preg_split("/\r?\n\s*\r?\n/", trim($content));

$questions = [];
foreach ($blocks as $block) {
    $lines = array_map('trim', explode("\n", $block));

    $question   = $lines[0];
    $options    = array_slice($lines, 1, 4); // A-D
    $answerLine = $lines[5] ?? '';           // ANSWER: X hoặc ANSWER: A,C

    // ---- XỬ LÝ NHIỀU ĐÁP ÁN ----
    preg_match('/ANSWER:\s*([A-D](?:,[A-D])*)/', $answerLine, $match);
    $correctList = [];

    if (!empty($match[1])) {
        $correctList = explode(",", $match[1]);  // Mảng đáp án đúng ["A","C"]
    }

    $questions[] = [
        "question" => $question,
        "options"  => $options,
        "correct"  => $correctList  // nhiều đáp án
    ];
}
?>
<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="UTF-8">
<title>Bài thi trắc nghiệm</title>
<style>
    body { font-family: Arial; width: 900px; margin: auto; }
    .question { border: 1px solid #ccc; padding: 10px; margin: 15px 0; border-radius: 6px; background: #f9f9f9; }
    .correct { color: green; font-weight: bold; }
    .wrong { color: red; font-weight: bold; }
    ul { padding-left: 0; list-style: none; }
</style>
</head>
<body>

<h2>📘 Bài thi trắc nghiệm</h2>

<form method="post">

<?php
$submitted = isset($_POST['submit']);
$score = 0;

foreach ($questions as $index => $q) {
    echo "<div class='question'>";
    echo "<h3>Câu ".($index+1).": " . htmlspecialchars($q['question']) . "</h3>";

    $correctList = $q['correct'];   // mảng: ["A","C"]
    $userAnswers = $_POST["q{$index}"] ?? [];

    echo "<ul>";

    foreach ($q['options'] as $i => $opt) {
        $letter = chr(65 + $i);  // A B C D
        $isChecked = in_array($opt, $userAnswers, true);

        // màu hiển thị
        $class = "";
        if ($submitted) {
            if (in_array($letter, $correctList)) {
                $class = "correct"; // đáp án đúng
            } elseif ($isChecked) {
                $class = "wrong"; // chọn sai
            }
        }

        $valueEscaped = htmlspecialchars($opt, ENT_QUOTES);
        $labelEscaped = htmlspecialchars($opt);
        $checkedAttr = $isChecked ? ' checked' : '';

        echo "<li class='$class'>
                <label>
                    <input type='checkbox' name='q{$index}[]' value='{$valueEscaped}'{$checkedAttr}>
                    {$labelEscaped}
                </label>
              </li>";
    }

    // Hiển thị đáp án sau khi nộp bài
    if ($submitted) {
        if (!empty($correctList)) {
            echo "<p><strong>Đáp án đúng:</strong> <span class='correct'>" . implode(", ", $correctList) . "</span></p>";

            // ---- CHẤM ĐIỂM: đúng khi TRÙNG KHỚP 100% ----
            $correctAnswersText = [];
            foreach ($correctList as $letter) {
                $correctIndex = ord($letter) - 65;
                $correctAnswersText[] = $q['options'][$correctIndex];
            }

            sort($correctAnswersText);
            sort($userAnswers);

            if ($correctAnswersText === $userAnswers) {
                $score++;
            }
        } else {
            echo "<p style='color:orange'><strong>Câu này không có đáp án chuẩn.</strong></p>";
        }
    }

    echo "</ul></div>";
}

// Hiển thị điểm
if ($submitted) {
    echo "<h2>Kết quả: $score / ".count($questions)." câu có đáp án</h2>";
}
?>

<input type="submit" name="submit" value="Nộp bài" style="padding:10px 25px; font-size:16px;">
</form>

</body>
</html>
